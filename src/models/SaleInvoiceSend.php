...
/**
 * SaleInvoiceSend Class Doc Comment
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 ...
 */
class SaleInvoiceSend implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'SaleInvoiceSend';
    ...
    {
        $this->setIfExists('delivery_method', $data ?? [], 'according_workflow');
    ...
    */
    ...
    public function setEmailText($email_text)
    {
        if (is_null($email_text)) {
            array_push($this->openAPINullablesSetToNull, 'email_text');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('email_text', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
